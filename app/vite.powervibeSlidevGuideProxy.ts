...
import { POWERVIBE_GUIDE_SLIDEV_PROXY_PREFIX } from "./src/components/powervibe/shell/powervibeSlidevGuideConstants";
...
export function rewriteSlidevGuideIndexHtml(html: string): string {
  const p = POWERVIBE_GUIDE_SLIDEV_PROXY_PREFIX;
...
function proxyMiddleware(targetPort: number) {
  return (
    ...
    if (!rawUrl.startsWith(POWERVIBE_GUIDE_SLIDEV_PROXY_PREFIX)) {
      next();
      return;
    }

    const parsed = new URL(rawUrl, "http://127.0.0.1");
    const rest = parsed.pathname.slice(POWERVIBE_GUIDE_SLIDEV_PROXY_PREFIX.length) || "/";
...
export function powervibeSlidevGuideProxyPlugin(options: { targetPort: number }): Plugin {
  const mw = proxyMiddleware(options.targetPort);
  return {
    name: "powervibe-slidev-guide-proxy",
    enforce: "pre",
