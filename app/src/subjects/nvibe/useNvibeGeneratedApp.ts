...
import { fetchNvibeApp, putNvibeAppSource } from "./nvibeAppApi";
...
export function useNvibeGeneratedApp(appId: MaybeRefOrGetter<string | null | undefined>) {
  ...
  const dirty = computed(() => source.value !== lastLoaded.value);
  ...
  async function apply(): Promise<boolean> {
    ...
    applying.value = true;
    error.value = null;
    const r = await putNvibeAppSource(id, source.value, { sourceOrigin: "manual_code_editor" });
