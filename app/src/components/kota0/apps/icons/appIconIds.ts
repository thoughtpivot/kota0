 ...
 * UI maps these to `@heroicons/vue/24/outline` — keep in sync with View.vue.
...
export type AppIconId = (typeof K0_APP_ICON_IDS)[number];

const SET = new Set<string>(K0_APP_ICON_IDS);

export function isAppIconId(s: string): s is AppIconId {
  return SET.has(s);
}

/** Stable icon per app when `app_icon` is missing on legacy Scribe rows (read path only). */
export function defaultAppIconId(appId: string): AppIconId {
  let h = 0;
