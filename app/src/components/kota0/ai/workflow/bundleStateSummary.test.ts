...
import { appendRuntimeError, clearRuntimeErrors } from "@/components/kota0/runtime/runtimeErrorStore";
...
describe("buildBundleStateSummary", () => {
  it("renders 'idle' state with no logs or runtime errors", async () => {
    const summary = await buildBundleStateSummary(APP);
