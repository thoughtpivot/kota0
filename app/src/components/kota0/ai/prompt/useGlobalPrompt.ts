...
import type GlobalPromptBar from "@/components/kota0/ai/prompt/GlobalPromptBar.vue";

/** A code-expand dialog is open — let Escape close that first, not the prompt bar. */
function codeDialogOpen(): boolean {
...
export function useGlobalPrompt() {
  ...
  const globalPromptBarRef = ref<InstanceType<typeof GlobalPromptBar> | null>(null);
