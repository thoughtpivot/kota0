/**
 * Workspace PowerVibe AI completion — uses repo-root `GEMINI_API_KEY` / `GEMINI_MODEL` (not bundle secrets).
 * Called from `POST /api/powervibe/apps/:appId/ai/complete`.
 */
import { GoogleGenAI } from "@google/genai";
import { DEFAULT_GEMINI_MODEL } from "@/lib/geminiModel";

/** Total UTF-8 byte budget for `prompt` + optional `systemInstruction`. */
export const POWERVIBE_PLATFORM_AI_MAX_INPUT_BYTES = 256 * 1024;

export type PowervibePlatformAiCompleteInput = {
  prompt: string;
  systemInstruction?: string;
  maxOutputTokens?: number;
};

export type PowervibePlatformAiCompleteResult =
  | { ok: true; text: string }
  | { ok: false; status: 503 | 502; error: string; message: string };

export function validatePowervibePlatformAiPayload(
  body: unknown,
): { ok: true; value: PowervibePlatformAiCompleteInput } | { ok: false; message: string; code: "bad_body" | "payload_too_large" } {
  ...
  if (inputBytes > POWERVIBE_PLATFORM_AI_MAX_INPUT_BYTES) {
    return {
      ok: false,
      code: "payload_too_large",
      message: `prompt + systemInstruction exceed ${POWERVIBE_PLATFORM_AI_MAX_INPUT_BYTES} UTF-8 bytes.`,
    };
  }
  return { ok: true, value: { prompt, ...(systemInstruction !== undefined ? { systemInstruction } : {}), ...(maxOutputTokens !== undefined ? { maxOutputTokens } : {}) } };
}

export async function runWorkspaceGeminiTextCompletion(input: PowervibePlatformAiCompleteInput): Promise<PowervibePlatformAiCompleteResult> {
  ...
  if (!apiKey) {
    return {
      ...
      message: "Workspace GEMINI_API_KEY is not set — configure repo-root .env for PowerVibe AI.",
    };
  }
