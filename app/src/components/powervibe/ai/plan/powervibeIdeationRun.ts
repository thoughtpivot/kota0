...
const POWERVIBE_RULES_COMPACT =
  ...
  "**Modern defaults:** Ship **`App.vue`** with **`<script setup lang=\"ts\">`** and **Composition API** (`ref`, `computed`, `watch`) — not legacy **Options API** unless the user asks. Prefer **`async`/`await`** with **`fetch(bundleApiUrl('api/powervibe-app/…'))`** then **`response.json()`** for bundle APIs (native **`fetch`**, not axios in **`App.vue`**). **PowerVibe platform AI (default for LLM text):** In **`App.backend.ts`**, prefer **`powervibePlatformAiCompleteText`** from **`@shared/powervibePlatformAi`** — **always** **`powervibePlatformAiCompleteText({ prompt: … })`** (object with **`prompt`**); **never** pass the user text as the sole argument (that produces a JSON **string** body and workspace Flight **`400`** strict-parse failure). It **`POST`**s **`/api/powervibe/apps/:appId/ai/complete`** on **`POWERVIBE_PLATFORM_API_ORIGIN`** with **`POWERVIBE_APP_ID`** from bundle **`.env`** (workspace repo-root **`GEMINI_API_KEY`** /**`GEMINI_MODEL`** — **no** bundle **`GEMINI_*`** for that flow). Same hosted-route trust boundary as **`/api/powervibe/transcribe-audio`** — **not** an npm client package like Flight/Scribe. **Direct Gemini (opt-in):** When the user wants **their own** Gemini key/model **in the bundle**, use **`@google/genai`**: **`import { GoogleGenAI } from '@google/genai'`** → **`new GoogleGenAI({ apiKey })`** → **`await ai.models.generateContent({ model, contents: [{ role: 'user', parts: [{ text: '…' }] }], config?: { temperature, maxOutputTokens } })`** → **`response.text`**. **Obsolete:** **`GoogleGenerativeAI`**, **`getGenerativeModel`**, **`model.generateContent(string)`**, **`result.response.text()`**. For direct mode use **`process.env.GEMINI_MODEL?.trim()`** when set; otherwise a **current** flash-class id — **do not** assume **`gemini-1.5-*`**. Node builtins: prefer **`node:`** imports (**`node:crypto`**, **`node:fs`**). " +
  ...
  "**`App.backend.ts` (Flight):** Flight `require()`s this file in Node, not Vite. **Never** `import` from `@/` or `~icons/…` — **`@shared/*` is allowed** (e.g. **`from '@shared/scribeRestClient'`**, **`from '@shared/scribeRowEnvelope'`**, **`from '@shared/powervibePlatformAi'`**). Otherwise use `from '@koa/router'`, **`import { bodyParser } from '@koa/bodyparser'`** then **`router.use(bodyParser())`** (not legacy **`koa-bodyparser`** require patterns), `from 'node:…'`, other packages from workspace `dependencies`, and relative files; end with `export default router.routes()` (same idea as the repo’s other `*.backend.ts` files). **LLM text:** prefer **`powervibePlatformAiCompleteText({ prompt: … })`** (**Modern defaults**); **`@google/genai`** only when the user opts into bundle **`GEMINI_*`**. **`@modelcontextprotocol/sdk`** (MCP servers) belongs **here** too; secrets **only** in bundle **`.env`**, never wired through `App.vue`. Koa change: **at most one** ```ts fence, **full file**, with **default export**. **Do not** register on `/api/powervibe/…` — use **`/api/powervibe-app/…`** (or another non-core prefix) as the **only** path shape for per-app APIs. " +
