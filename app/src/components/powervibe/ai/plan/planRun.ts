...
const PLAN_SYSTEM =
  ...
  "**Modern stack:** Vue **`<script setup lang=\"ts\">`** + Composition API; **`fetch`** from the browser to **`/api/powervibe-app/…`** (via bundle helper). Gemini in backends: **`GoogleGenAI`** from **`@google/genai`** + **`ai.models.generateContent`** + **`response.text`** — not **`GoogleGenerativeAI`**/**`getGenerativeModel`**. Model id from **`GEMINI_MODEL`** env when possible; avoid **`gemini-1.5-*`** as the assumed default. " +
  ...
  "When the idea mentions **AI**, **LLM**, **sentiment**, **insights**, **summarize**, or **analyze text**, plan **`@google/genai`** on **`App.backend.ts`** + **`GEMINI_API_KEY`**—not fake static copy in the UI. For **canvas/generative/abstract** visuals, plan **canvas/SVG**—not generic charts unless charts are clearly part of the ask. " +
  ...
  "**Gemini (`@google/genai`)** and **MCP (`@modelcontextprotocol/sdk`)** belong in **`App.backend.ts`** with secrets in bundle env when relevant—not called from untrusted browser code. When discussing bundle **`.env`**, plans should treat **full `KEY=value` visibility** as normal for the builder—no redaction in assistant prose. " +
