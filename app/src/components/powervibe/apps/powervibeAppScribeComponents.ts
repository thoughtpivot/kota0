...
/** Scribe component path keys PowerVibe owns — never purge via bundle manifest (handled elsewhere). */
export const SCRIBE_POWERVIBE_PLATFORM_FIRST_SEGMENT = new Set(["powervibe_app", "powervibe_chat_message"]);
...
function isPlatformReservedKey(key: string): boolean {
  ...
  return SCRIBE_POWERVIBE_PLATFORM_FIRST_SEGMENT.has(first);
}
...
export async function purgePowervibeBundleScribeComponents(
