#!/usr/bin/env node
/**
 * Smoke: Koa Kota0 routes + materialized files (via diagnostics).
 * Run with the dev app up: `npm run start:app` and Vite (or set K0_SMOKE_BASE to your UI origin so /api is proxied).
 *
 * @example
 *   K0_SMOKE_BASE=http://127.0.0.1:3001 node scripts/kota0-smoke.mjs
 */

const base = (process.env.K0_SMOKE_BASE || "http://127.0.0.1:3001").replace(/\/$/, "");
...
async function main() {
  console.log(`K0_SMOKE_BASE=${base}\n`);

  const d = await json("/kota0/diagnostics");
  if (d.status !== 200) {
    console.error("FAIL diagnostics", d.status, d.u, d.body);
    if (d.status === 0) {
      console.error("Is the app running? `npm run start:app` — set K0_SMOKE_BASE to the embedded Vite origin (default 3001) or Koa+api.");
    }
    process.exitCode = 1;
    return;
  }
  ...
  if (!d.body.appVueExists || !d.body.appBackendExists) {
    console.warn("WARN: materialized files missing; open an Kota0 app in the UI or GET /api/kota0/apps/:id");
  }

  const apps = await json("/kota0/apps");
  if (apps.status !== 200) {
    console.error("FAIL GET /api/kota0/apps", apps.status, apps.body);
    process.exitCode = 1;
    return;
  }
  ...
  const one = await json(`/kota0/apps/${encodeURIComponent(id)}`);
  if (one.status !== 200) {
  ...
  const msg = await json(`/kota0/apps/${encodeURIComponent(id)}/messages`);
  if (msg.status !== 200) {
