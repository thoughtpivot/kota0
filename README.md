...
<p align="center">
  <img src="docs/screenshots/kota0-workspace-overview.png" alt="Kota0 workspace overview: apps rail, AI chat, and live Preview for a generated app" width="92%" />
</p>

<p align="center">
  <img src="docs/screenshots/kota0-preview-tab.png" alt="Preview tab with generated dashboard in the iframe" width="30%" />
  &nbsp;&nbsp;
  <img src="docs/screenshots/kota0-code-tab.png" alt="Code tab with Frontend editor" width="30%" />
  &nbsp;&nbsp;
...
On first load, `**/`** is the workspace: an apps rail, AI panel, **Preview** and **Code** tabs (Frontend, Backend, **Secrets** for per-app `bundles/<appId>/.env`), and the active app’s materialized `App.vue` / `App.backend.ts` (written to `bundles/<appId>/`; the workspace keeps an `[App.vue](app/src/components/kota0/viewer/generated/App.vue)` mirror for its own tooling). If no apps exist yet, the UI creates a default app.
...
- **Governed environments** — Scribe-backed apps and chat (`k0_app`, `k0_chat_message`), optional **SSE** streaming (`VITE_K0_CHAT_STREAM`), and **voice → transcript → send** via Gemini (`[geminiTranscribeAudio.ts](app/src/components/kota0/ai/audio/geminiTranscribeAudio.ts)`) for field-style input.
...
| Route   | What you get                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `**/`** | **Kota0 workspace** — apps rail (multiple generated apps), resizable **AI** panel (Gemini chat and **Apply** when the model returns a valid Vue SFC), **Preview** (live iframe of the materialized app), and **Code** (edit `App.vue` and `App.backend.ts` with Apply). See `[kota0.vue](app/src/components/kota0/kota0.vue)` and `[WorkspaceViewer.vue](app/src/components/kota0/viewer/workspace/WorkspaceViewer.vue)`. |
...
| Area                                                                                                                                         | Role                                                                      |
| -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `[shell/](app/src/components/kota0/shell/)`, `[WorkspaceLayout.vue](app/src/components/kota0/shell/WorkspaceLayout.vue)` | Header chrome and grid layout                                             |
...
- **AI** uses [`Kota0.backend.ts`](app/src/components/kota0/Kota0.backend.ts) with a Mastra-backed flow on `POST /api/kota0/apps/:id/messages/stream`, in one of two server-selected modes (`K0_AI_MODE`, see `[.env.example](.env.example)`): **`oneshot`** *(default)* — a single model call returns markdown with one full code fence that renders **inline in chat** and is auto-applied (the fast path); **`agentic`** — **classify → optional plan → auto-apply** through tools (the agent writes files via tools, so code is not echoed inline). Set `VITE_K0_CHAT_STREAM=1` in `.env` (default-on unless `0`/`false`). Plan cards are informational; apply runs automatically. Manual fence apply from chat messages still uses **Code** dialogs → `PUT /api/kota0/apps/:id`.
...
| **Kota0 app bundle (preview)** | **4000**                 | Per-app Flight **production**: `vite build` + static `**dist/`** + `App.backend.ts` on one listener. Not running until you open **Apply** / load an app (supervisor in `[bundleRunner.ts](app/src/components/kota0/deploy/runner/bundleRunner.ts)`). |
...
| **Scribe Gateway**                 | **3002**                 | Token → per-app table-prefix proxy in front of Scribe for **bundle** code (preview + deployed). `SCRIBE_GATEWAY_PORT`; started by `start:workspace` / `npm run start:gateway` (`[gateway/](app/src/components/kota0/gateway/)`).                                       |
...
| `**K0_AI_MODE`**                           | `**oneshot`** (default) — one fast model call, code rendered **inline in chat** + auto-applied; or `**agentic`** — classify → optional plan → tool-driven apply. See `[.env.example](.env.example)`.                       |
...
| Command                    | Description                                                                                                                                                                        |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run start:workspace`  | **Recommended for local dev:** runs **`start:docker`**, **`start:app`**, **`start:slides`**, and **`start:gateway`** together via **`concurrently`** (`-n` / `-c` for readable logs). Easiest way to bring up the full Kota0 workspace; use the commands below individually whenever you want. (`npm run restart` = `kill:ports` then `start:workspace`.) |
| `npm run start:docker`     | **`docker compose up -d`** (detached) — Redis **6379**, Postgres **5432**, Scribe **1337** (`[compose.yml](compose.yml)`).                                                                 |
...
- `**npm run k0:smoke**` — diagnostics + list apps + one app + messages; needs the Compose stack and Flight up (`**npm run start:workspace**`, or `**npm run start:docker**` plus `**npm run start:app**`).
...
| Kota0 HTTP API | `[Kota0.backend.ts](app/src/components/kota0/Kota0.backend.ts)` — `**/api/kota0/apps`** (CRUD), `**…/messages/stream**` (chat workflow), `**…/ai/stats**`, `**…/source-revisions**`. **Scribe is source of truth**; active app written to `**bundles/<appId>/`** + `**generated/App.vue`** mirror; bundle Flight restart via `[bundleRunner.ts](app/src/components/kota0/deploy/runner/bundleRunner.ts)`. Successful **PUT** sets `**active`** when needed; AI **Apply** then **PATCH**es `**applied`**. Dev: `[appApi.ts](app/src/components/kota0/apps/data/appApi.ts)` uses same-origin `**/api/...`** ( `**VITE_KOA_ORIGIN**` only if bypassing proxy). |
| AI workflow      | `[chatWorkflow.ts](app/src/components/kota0/ai/workflow/chatWorkflow.ts)` — `agentic` mode: classify → plan (complex) → auto-apply via Mastra. `oneshot` (default) is a single model call with code rendered inline in chat; select via `K0_AI_MODE`. |
