...
- [`app/src/components/home/Home.vue`](app/src/components/home/Home.vue) already has: sticky nav, dark hero, directory table with `fetchNvibeApps`, `NvibeAppStatusBadge`, `formatNvibeAppUpdatedAt`, `goNvibe` / `openAppInWorkspace` / `scrollToApps`, and three “pillar” columns.
- Dependencies [`chart.js`](https://github.com/chartjs/Chart.js) and [`vue-chartjs`](https://github.com/apertureless/vue-chartjs) exist in the root [`package.json`](package.json) (same stack as the nVibe preview; see [`chartJsSetup.ts`](app/src/components/nvibe/viewer/preview/chartJsSetup.ts) for registration patterns).
...
Apply the prompt palette **on the home route only** (wrapper on the root `div` of `Home.vue`), so the rest of the app (`/` nVibe) keeps existing tokens.
...
- Do **not** load Chart.js from a CDN; bundle only (project rule and [`nvideIdeationRun` guidance](app/src/components/nvibe/ai/plan/nvibeIdeationRun.ts)).
