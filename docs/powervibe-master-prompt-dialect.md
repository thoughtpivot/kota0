# PowerVibe “master prompt” dialect (maintainer / integration note)

This file is **not** shown in the PowerVibe product UI — it documents how to reconcile prompts authored elsewhere (e.g. long Gemini thread specs) with how **`App.vue`** is built here.

External prompts often read like **product specs**: role, palette, narrative beats, chart types, and interactions. The **ideation model** is steered in [`powervibeIdeationRun.ts`](../app/src/components/powervibe/ai/plan/powervibeIdeationRun.ts) to ship **dense, polished** single-file UIs without requiring users to paste a template.

## Charts

| External / Gemini-style wording | In PowerVibe `App.vue` |
| --- | --- |
...
- In `<style>`, **`@apply`** with **`selection:`** utilities breaks the build — use plain **`::selection { … }`** (see PowerVibe rules) or `selection:` classes on template nodes only.
...
2. Keep **one** narrative app in **one** SFC; multi-file pipelines are out of scope for stock PowerVibe.
